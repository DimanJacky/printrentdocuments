...
const WordProcessor = () => {
  ...
  const [replacementComment, setReplacementComment] = useState(
    'Od momenta prijema depozita, imovina se uklanja iz zakupa, a Zakupodavac dostavlja Zakupcu elektronsku kopiju dokumenta kojim se potvrđuje vlasništvo nad imovinom ponuđenom za iznajmljivanje.'
  );
  const [replacementConclude, setReplacementConclude] = useState(
    `12 meseci od ${formatDateTodayYY()} do ${addOneYear(formatDateTodayYY())}`
  );
